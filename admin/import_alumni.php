...
if (isset($_POST['import'])) {
    ...
    if (isset($_FILES['excel_file']['name']) && in_array($_FILES['excel_file']['type'], $file_mimes)) {
        ...
        $sql_alumni = "INSERT INTO alumnus_bio (firstname, middlename, lastname, gender, batch, course_id, email, connected_to, avatar, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        if ($stmt_alumni === false || $stmt_users === false) {
...
foreach ($sheetData as $row) {
    ...
    $stmt_alumni->bind_param("sssssissss", $firstname, $middlename, $lastname, $gender, $batch, $course_id, $email, $connected_to, $avatar, $status);
    if (!$stmt_alumni->execute()) {
        echo "Error executing statement for $firstname: " . $stmt_alumni->error . "<br>";
        continue; // Skip this row if there's an error
    }

    // Get the newly inserted alumni ID
    ...
    $type = 2; // Alumni user type
    $auto_generated_pass = $temp_password;

    // Insert into users table
    $stmt_users->bind_param("sssssi", $name, $username, $hashed_password, $type, $auto_generated_pass, $alumnus_id);
